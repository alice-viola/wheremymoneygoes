...
class DatabaseService {
  ...
  async createUser(userData) {
    ...
    stmt.run(id, userData.email, userData.username || null);
  ...
  async getAccount(accountId, userId) {
    const stmt = this.db.prepare('SELECT * FROM accounts WHERE id = ? AND user_id = ?');
  ...
  async setDefaultAccount(accountId, userId) {
    ...
    const setDefault = this.db.transaction(() => {
      ...
      const unsetStmt = this.db.prepare('UPDATE accounts SET is_default = 0 WHERE user_id = ?');
      unsetStmt.run(userId);
      
      // Set the new default
      const setStmt = this.db.prepare('UPDATE accounts SET is_default = 1 WHERE id = ? AND user_id = ?');
      const result = setStmt.run(accountId, userId);
  ...
  async unsetOtherDefaults(userId, exceptAccountId) {
    const stmt = this.db.prepare('UPDATE accounts SET is_default = 0 WHERE user_id = ? AND id != ?');
    stmt.run(userId, exceptAccountId);
  }
  ...
  async getTransaction(transactionId, userId) {
    const stmt = this.db.prepare('SELECT * FROM transactions WHERE id = ? AND user_id = ?');
  ...
  async bulkUpdateTransactions(userId, transactionIds, updates) {
    const results = [];
    
    const bulkUpdate = this.db.transaction(async () => {
      for (const transactionId of transactionIds) {
        const updated = await this.updateTransaction(transactionId, userId, updates);
        if (updated) {
  ...
  async getAnalyticsSummary(userId, filters = {}) {
    ...
    `);
    const income = incomeStmt.get(...params).total;
    ...
    const expenseStmt = this.db.prepare(`
      ...
      WHERE user_id = ? AND kind = '-' ${dateFilter}
    `);
    const expenses = expenseStmt.get(...params).total;
    ...
    `);
    const transactionCount = countStmt.get(...params).count;
    ...
    `);
    const categoriesCount = categoriesStmt.get(...params).count;
  ...
  async getUpload(uploadId, userId) {
    const stmt = this.db.prepare('SELECT * FROM uploads WHERE id = ? AND user_id = ?');
    return stmt.get(uploadId, userId);
  }
  ...
  async deleteUpload(uploadId, userId) {
    ...
    const deleteTransactions = this.db.prepare('DELETE FROM transactions WHERE upload_id = ? AND user_id = ?');
    deleteTransactions.run(uploadId, userId);
    
    // Delete the upload record
    const stmt = this.db.prepare('DELETE FROM uploads WHERE id = ? AND user_id = ?');
    const result = stmt.run(uploadId, userId);
    return result.changes > 0;
  }
  ...
  async getMerchantCategory(merchant) {
    const stmt = this.db.prepare('SELECT category FROM merchant_cache WHERE merchant = ?');
    const result = stmt.get(merchant);
