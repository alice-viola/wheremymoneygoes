    ...
    <div class="relative z-10 max-w-md w-full space-y-8 px-4 sm:px-6 lg:px-8">
      ...
      <div class="card p-8">
        ...
        <p class="text-center text-sm text-gray-600 dark:text-gray-400 mb-6">
          Or
          <router-link to="/signup" class="font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300 transition-colors">
...
const handleLogin = async () => {
  ...
  try {
    await authStore.login(formData.value.email, formData.value.password, rememberMe.value)
    
    // Small delay to ensure stores are properly initialized
    ...
    error.value = err.response?.data?.error || 'Failed to sign in. Please check your credentials and try again.'
  } finally {
    loading.value = false
  }
}
